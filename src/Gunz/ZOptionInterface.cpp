...
void ZOptionInterface::InitInterfaceOption(void)
{
...

	{
		...
		char CrosshairScale[64];
		sprintf_safe(CrosshairScale, "%.2f", Cfg.GetCrosshairScale());
		...
		std::pair<const char*, const char*> Edits[] = {
			{"FOVOption",                 StringFOV},
			{"CrossHairScaleOption",      CrosshairScale},
		...
		if (auto&& Widget = ZFindWidget("CrosshairScaleOption"))
		{
			Widget->Enable(true);
		}
...
#endif
			{ "CrosshairScaleOption", [](const char* Value) {
				ZGetConfiguration()->GetEtc()->fCrosshairScale = atof(Value); } },
